...
- The model outputs a prediction $\widehat{y_{i}} = f(x_{i})$.
  ...
  - If we assume that the error follows a normal distribution, we have
    ...
    $p(y_{i}\ |\ x_{i},\ \theta)\  = \frac{1}{\sqrt{2\pi\sigma^{2}}}e^{- \frac{{(y_{i} - \widehat{y_{i}})}^{2}}{2\sigma^{2}}}$

- Thus, $L(\theta)\  \propto \ \prod_{i = 1}^{n}\frac{1}{\sqrt{2\pi\sigma^{2}}}e^{- \frac{{(y_{i} - \widehat{y_{i}})}^{2}}{2\sigma^{2}}}$

- And maximizing the expression above is equivalent to minimizing ${(y_{i} - \widehat{y_{i}})}^{2}$

- Taking the $\log$ of the expression above gives

  $\sum_{i = 1}^{n}\log\frac{1}{\sqrt{2\pi\sigma^{2}}} - \frac{{(y_{i} - \widehat{y_{i}})}^{2}}{2\sigma^{2}}$
