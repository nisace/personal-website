---
title: "[]{#_oi9bdj8ytytd .anchor}Information theory"
---
...
- The entropy $H_{b}(X)$ in base $b$ is the lower bound of the expected length of any code with an alphabet size of $b$ that encodes events of $X$.
  ...
  - See [[Relationship to encoding]{.underline}](#relationship-to-encoding) for more details.
...
+-------------------+------------------+-----------------------------------------------------------------+---------------+
| Self information  | \- Level of      | $I(x) = - \log_{b}(p(x))$                                       | Individual    |
|                   | \"surprise\" of  |                                                                 | event $x$     |
|                   | event $x$.       |                                                                 |               |
+-------------------+------------------+-----------------------------------------------------------------+---------------+
| Entropy           | \- Expected      | $H(X) = \ \sum_{\chi}^{} - p(x)\ log(p(x))$                     | Random        |
|                   | value of         |                                                                 | variable $X$  |
|                   | information      |                                                                 |               |
|                   | content.\        |                                                                 |               |
|                   | - Lower bound of |                                                                 |               |
...
In a supervised classification problem with $C$ classes, the following loss, called the [[cross entropy loss]{.underline}](https://scikit-learn.org/stable/modules/model_evaluation.html#log-loss), is often used
...
A mnemonic way to remember which of the two expressions above is the forward $D_{KL}$ or the reversed $D_{KL}$ is explained is this [[blog post]{.underline}](https://mpatacchiola.github.io/blog/2021/01/25/intro-variational-inference.html):
